...
#
# Usage: ./best_hit_from_kraken_noconfig.sh -i path_to_sample_folder -r pre/post(relative to assembly)  -t paired|assembled|assembled_BP(source_type) -k kraken|kraken2(source)
#
...
show_help () {
	echo "Usage is ./best_hit_from_kraken_noconfig.sh  -i path_to_sample_folder  -r pre|post -t paired|assembled|assembled_BP -k kraken|kraken2"
...
options_found=0
while getopts ":h?i:r:t:k:" option; do
...
if [[ "${options_found}" -eq 0 ]]; then
	...
	echo "Relativity must equal either pre or post, exiting best_hit_from_kraken_noconfig.sh"
elif [[ "${type}" != "paired" ]] && [[ "${source}" != "assembled" ]] && [[ "${source}" != "assembled_BP" ]]; then
	echo "Source must be paired, assembled, or assembled_BP, exiting best_hit_from_kraken_noconfig.sh"
elif [[ "${source}" != "kraken" ]] && [[ "${source}" != "kraken2" ]]; then
	echo "Source must be kraken or kraken2, exiting best_hit_from_kraken_noconfig.sh"
fi
