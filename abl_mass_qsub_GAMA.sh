...
if [[ ! -f "${list}" ]]; then
	...
	echo "Clobberness was not input, be sure to add keep or clobber ...exiting"
	exit 1
elif [[ ! -z "${alt_db}" ]]; then
	if [[ ! -f "${alt_db}" ]]; then
	...
	if [ ${counter} -lt ${max_subs} ]; then
		...
		if [[ ! -f "${processed}/${project}/${sample}/GAMA/${sample}_${ResGANNCBI_srst2_filename}.GAMA" ]]; then
			...
			if [[ "${use_alt_db}" == "true" ]]; then
				echo -e "\"${shareScript}/run_GAMA.sh\" -n \"${sample}\" -p \"${project}\" -k -c \"${config}\" -d \"${database_path}\"" >> "${main_dir}/GAMAAR_${sample}_${start_time}.sh"
			else
